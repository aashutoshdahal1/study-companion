...
import { X, Upload, Loader2, FileAudio, ClipboardPaste, RotateCcw, ChevronDown, ChevronUp } from "lucide-react";
...
async function startTranscription(fileLink: string, filename: string): Promise<string> {
  const res = await fetch(`${AC_BASE}/transcribe`, {
    ...
    body: JSON.stringify({ file_link: fileLink, filename }),
  });
...
function buildTranscript(data: any): string {
  ...
  if (words.length === 0) return plainText;

  // Group consecutive words by speaker into segments
  const segments: { speaker: string; start: number; words: string[] }[] = [];
  let current: { speaker: string; start: number; words: string[] } | null = null;
  ...
  for (const w of words) {
    ...
    if (!current || current.speaker !== speaker) {
      ...
      current = { speaker, start: w.start ?? 0, words: [text] };
    } else {
  ...
  // Format speaker labels: "speaker_0" → "Speaker 1", etc.
  ...
  return segments
    .map((seg) => {
      ...
      const text = seg.words.join(" ").replace(/ ([,।?!])/g, "$1");
      return `${time}\n${label}\n${text}`;
    })
    .join("\n\n");
}
...
export function AudioTranscriber({ isOpen, onClose, onInsertToNotes }: AudioTranscriberProps) {
            ...
            >
              ...
              {token && (
                <button
                  ...
                  onClick={(e) => { e.stopPropagation(); setShowTokenInput(true); }}
                >
                  Change token
                </button>
              )}
